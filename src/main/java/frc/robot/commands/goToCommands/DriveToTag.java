...
public class DriveToTag extends Command {
    ...
    private final ProfiledPIDController driveController = new ProfiledPIDController(AutonConstants.drivekP, 0.0,
            AutonConstants.drivekD,
            new TrapezoidProfile.Constraints(AutonConstants.driveMaxVelocity,
                    AutonConstants.driveMaxAcceleration),
            0.02);

    private final ProfiledPIDController thetaController = new ProfiledPIDController(AutonConstants.thetakP, 0.0,
            AutonConstants.thetakD,
            new TrapezoidProfile.Constraints(AutonConstants.thetaMaxVelocity,
                    AutonConstants.thetaMaxAcceleration),
            0.02);
    ...
    public DriveToTag(Drive drive, Supplier<Pose2d> robotPose, Supplier<Pose2d> targetPose) {
        ...
        driveController.setTolerance(AutonConstants.driveTolerance);
        thetaController.setTolerance(AutonConstants.thetaTolerance);
    ...
    public boolean atGoal() {
        return Math.abs(dtheta) < AutonConstants.thetaTolerance
                && Math.abs(dp) < AutonConstants.driveTolerance;
    }
